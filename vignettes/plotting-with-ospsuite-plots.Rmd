...
plotTimeProfile(
  myDataCombined,
  mapping = ggplot2::aes(color = dataType)
)
```
...
plotTimeProfile(myDataCombined, xUnit = "min")
```

```{r units-yunit}
# Override y-axis unit: display concentration in µg/l instead of µmol/l
plotTimeProfile(myDataCombined, yUnit = "µg/l")
```

The same parameters work for all other plotting functions:

```{r units-predvsobs}
# Predicted vs Observed with custom x-axis unit
plotPredictedVsObserved(myDataCombined, xUnit = "min")
```

```{r units-residuals}
# Residuals vs Time with custom x-axis unit
plotResidualsVsCovariate(myDataCombined, xAxis = "time", xUnit = "min")
```

Passing units directly is equivalent to pre-converting the data with `convertUnits()`:

```{r units-equivalent, eval=FALSE}
# These two calls produce identical plots:
plotTimeProfile(myDataCombined, xUnit = "min")

plotTimeProfile(convertUnits(myDataCombined, xUnit = "min"))
```
