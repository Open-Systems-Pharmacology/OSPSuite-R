...
The settings that were in place before `setDefaults()` was called are stored in `ospsuiteEnv$ospSuitePlotsDefaults`. You can restore them at any time using `getOSPSuiteSetting()`, which provides access to any named setting stored in `ospsuiteEnv`:

```r
ospsuite.plots::resetDefaults(getOSPSuiteSetting("ospSuitePlotsDefaults"))
```
