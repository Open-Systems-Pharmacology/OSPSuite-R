...
.ospRuntimeAvailable <- isTRUE(tryCatch(
  requireNamespace("rSharp", quietly = TRUE) &&
    "dotnetAvailable" %in% getNamespaceExports("rSharp") &&
    rSharp::dotnetAvailable(),
...
The plotting functions style each plot individually, so no global setup is required and loading `ospsuite` leaves your `ggplot2` theme and geom defaults untouched. The only optional setting is the watermark, which must be configured before creating plots:
...
```{r timeprofile-legend-per-dataset, eval=.ospRuntimeAvailable}
