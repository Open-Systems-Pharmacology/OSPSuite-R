...
plotTimeProfile(myPopDataCombined)
```
...
plotTimeProfile(myPopDataCombined, aggregation = "quantiles", quantiles = c(0.05, 0.5, 0.95))
```

```{r timeprofile-arithmetic, eval=FALSE}
# Or use arithmetic mean with standard deviations
plotTimeProfile(myPopDataCombined, aggregation = "arithmetic", nsd = 1)
```

### Custom Aesthetic Mappings

You can customize the plot appearance using `ggplot2` aesthetic mappings. By default, plots created with these functions display a legend with no title, positioned in the plot area.
