...
Before creating plots with `ospsuite.plots`, it's important to initialize the plotting environment properly. When `ospsuite` is loaded, `ospsuite.plots::setDefaults()` is called automatically. You may additionally configure the watermark option:
...
The `setDefaults()` function initializes various plotting defaults that ensure consistent appearance across all plots, and is called automatically on package load. The watermark option allows you to add a watermark to your plots.  
