...
All plotting functions in this vignette make use of the `{ospsuite.plots}` library. 
...
2. **Configure the watermark option** using `options(ospsuite.plots.watermark_enabled = TRUE)` or `options(ospsuite.plots.watermark_enabled = TRUE)` 
...
You can customize the quantiles or use different aggregation methods:

```{r timeprofile-aggregation}
# Specify custom quantiles (5th, 50th, and 95th quantiles)
plotTimeProfile(myPopDataCombined, yScale = 'log', aggregation = "quantiles", quantiles = c(0.05, 0.5, 0.95))
```

```{r timeprofile-arithmetic, eval=FALSE}
# Or use arithmetic mean with standard deviations
plotTimeProfile(myPopDataCombined, yScale = 'log', aggregation = "arithmetic", nsd = 1)
```

### Custom Aesthetic Mappings

By default the legend is created by the variable  `group` of the dataCombined Object.
...
This function is a wrapper around ggsave which adjust the height (if `height = NULL').
