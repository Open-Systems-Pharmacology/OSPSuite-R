...
In some cases, the user might want want to run multiple simulations in parallel. This can be achieved easily by simply passing a list of simulations to run.
...
```{r MultipleSimResultsExport}
# Load and run multiple simulations concurrently. 
simFilePath <- system.file("extdata", "Aciclovir.pkml", package = "ospsuite")

# We load 3 times the same simulation for convenience. But in real life scenarios, they would be different simulations
...
The results in this case will be a list of `SimulationResults`-object , e.g. one for each simulation.
