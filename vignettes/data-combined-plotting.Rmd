...
Observed versus simulated plots show allow to assess how far simulated results 
...
The identity line represents perfect correspondance of simulated values with the 
observed ones. By default, a "two-fold" are is marked by the dashed lines. 
The "x-fold" range is defined as values that are `x`-fold above and `1/x`-fold 
below the observed onces. The user can specify multiple ranges by the `foldDistance` 
argument.
...
Each of the `plotXXX()` returnsan a `ggplot2` object. Lets create different plots 
