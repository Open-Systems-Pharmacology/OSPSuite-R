...
```{r ParallelRun, eval=.ospRuntimeAvailable}
library(ospsuite)
# Load and run the simulation
simFilePath <- system.file("extdata", "Aciclovir.pkml", package = "ospsuite")
...
```{r BatchRunInit, eval=.ospRuntimeAvailable}
library(ospsuite)
simFilePath <- system.file("extdata", "Aciclovir.pkml", package = "ospsuite")
...
```{r BatchRunEnque, eval=.ospRuntimeAvailable}
...
```{r BatchRunRun, eval=.ospRuntimeAvailable}
...
```{r BatchRunRunNewValues, eval=.ospRuntimeAvailable}
...
```{r BatchRunStateVarMolecule, eval=.ospRuntimeAvailable}
stateVariableParam <- getParameter(path = "Organism|Lumen|Stomach|Liquid", container = sim1)
