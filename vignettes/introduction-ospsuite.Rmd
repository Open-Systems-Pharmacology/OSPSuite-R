...
populationResults$print()
```
By default, population simulations are run in parallel on multi-core machines - one core executes a separate individual. By default, the maximal number of cores that can be used equals the number of available (logical) cores minus one. The user can change the default behavior by providing custom `SimulationRunOptions`.
...
Accessing simulated values for a population simulation is similar to accessing the results of an individual simulation. The values of `IndividualId`, `Time`, and the simulated outputs, are appended for each simulated invidual. Note that this results in non-monotonously increasing column `Time`.
...
doseParam <- getParameter(doseParamPath, container = sim)
...
allAciclovirMolecules <- getAllMoleculesMatching("Organism|**|Aciclovir", container = sim)
...
The current simulation interval (i.e., the simulation times at which results are stored) be viewed by retrieving the property `output` of the `Simulation` object
...
####DOES NOT WORK YET BECAUSE OF A BUG!!!
...
liverVolume <- getParameter("Organism|Liver|Volume", container = sim)
liverVolume$print()
