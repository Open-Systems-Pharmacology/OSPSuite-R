...
icBBDf <- initialConditionsToDataFrame(icBBs[[1]])
# Print the first few rows of the data frame
head(icBBDf)

pvBBDf <- parameterValuesToDataFrame(pvBBs[["Rat"]])
# Print the first few rows of the data frame
head(pvBBDf)
...
-   `setInitialConditionsBB()`: Allows adding new entries or changing the values of existing entries in the Initial Conditions BB.
-   `deleteInitialConditionsBB()`: Allows deleting entries from the Initial Conditions BB.
-   `extendInitialConditions()`: Extending an IC BB by entries for specified molecules in all physical containers of a spatial structure.

Followning functionalities are available for the Parameter Values (PV) BBs:

-   `setParameterValuesBB()`: Allows adding new entries or changing the values of existing entries in the PV BB.
-   `deleteParameterValuesBB()`: Allows deleting entries from the PV BB.
-   `addLocalMoleculeParameters()`: Extending a PV BB by entries for local parameters of specified molecules in all physical containers of a spatial structure.
