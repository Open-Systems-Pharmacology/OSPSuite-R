...
A module's `Molecules` building block is exposed via `MoBiModule$getMoleculesBB()`, which returns a `MoleculesBuildingBlock` object. The methods below let you query molecule names without inspecting the underlying .NET object directly, which is useful when extending other building blocks (initial conditions, parameter values, expression parameters) for the molecules defined in the module:
