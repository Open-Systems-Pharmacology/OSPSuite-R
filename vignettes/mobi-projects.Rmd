...
### Creating several simulations at once

`createSimulations()` creates one simulation per entry of a named list of simulation configurations, the names of the list becoming the names of the created simulations. All simulations are created in a single call into the MoBi engine, which builds them in parallel.
...
A configuration that cannot be created does not fail the whole batch: a warning is shown and the entry of that simulation in the returned list is `NULL`. Pass `stopIfFails = TRUE` to raise an error instead.
