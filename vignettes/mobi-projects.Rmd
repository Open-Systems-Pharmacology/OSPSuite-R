...
print(myModule$parameterValuesBBnames)

# `removeBuildingBlock()` removes single-type building blocks (Molecules,
# Reactions, Spatial Structure, Passive Transports, Observers, Event Groups)
# by their type name, e.g.:
moleculesBB <- myProject$getModules("Thyroid")[[1]]$getMoleculesBB()
myModule$addBuildingBlocks(moleculesBB)
myModule$removeBuildingBlock(BuildingBlockTypes$Molecules)
```

A module can also be created with its building blocks in one call:

```{r createMoBiModuleWithBBs, eval = FALSE}
pvBB <- loadBuildingBlockFromPKML(
  filePath = "ParameterValues.pkml",
  type = BuildingBlockTypes$`Parameter Values`
)
icBB <- loadBuildingBlockFromPKML(
  filePath = "InitialConditions.pkml",
  type = BuildingBlockTypes$`Initial Conditions`
)
myModule <- createMoBiModule(
  "MyCustomModule",
  buildingBlocks = list(pvBB, icBB)
)
```
