---
title: "PKSim Installation"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{PKSim_installation}
...
and PKSim dependencies are included in the package. Therefore, there is no need
to install extra software to use `{ospsuite}`.

Also, `{ospsuite}` is fully self contained and is independant from any  version 
of OSPSuite and PKSim installed on the user computer.
...
It is necessary to have a PKSim installation that matches the version of 
`{ospsuite}` (*i.e* to use `{ospsuite}` v11.y.z, you need to have PKSim v11 installed).
...
Follow the steps from the previous section to download and extract PKSim to 
