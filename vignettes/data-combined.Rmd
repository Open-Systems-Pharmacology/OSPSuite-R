...
There are three ways to calculate the distance (residuals) - linear, logarithmic, and ratio - 
specified by the `scaling` argument. 

For **linear scaling** (`scaling = tlf::Scaling$lin` or `"linear"`):

> Residuals are calculated as: Simulated value - Observed value. This means that the residuals are defined by absolute differences.

For **logarithmic scaling** (`scaling = tlf::Scaling$log` or `"log"`):

> Residuals are calculated as: log(Simulated value) - log(Observed value) = log(Simulated value / Observed value). This means that the ratio of values is considered which is independent of the magnitude of the value. 
> 
> **Note:** For zero values, log(0) produces NaN (not a number). These points will be automatically excluded from the results, and a warning will be issued indicating how many points were affected.

For **ratio scaling** (`scaling = "ratio"`):

> Residuals are calculated as: Simulated value / Observed value. Values around 1.0 indicate good model fit, values < 1.0 indicate the model under-predicts, and values > 1.0 indicate the model over-predicts.
> 
> **Note:** Division by zero (when observed value is 0) produces Inf. These points will be automatically excluded from the results, and a warning will be issued.


```{r}
# Linear residuals
calculateResiduals(myDataCombined, scaling = ospsuite.plots::ResidualScales$linear)$residualValues

# Logarithmic residuals
calculateResiduals(myDataCombined, scaling = ospsuite.plots::ResidualScales$log)$residualValues
```
...
```{r}
# Linear residuals
totalError <- sum(abs(calculateResiduals(myDataCombined, scaling = ospsuite.plots::ResidualScales$linear)$residualValues))
