...
The inverse operation - creating `DataSet` objects from a `data.frame` - is
performed by `dataSetsFromDataFrame()`. The function accepts a `data.frame`
with the same structure as returned by `dataSetToDataFrame()` and returns a
named list of `DataSet` objects. This is useful for constructing `DataSet`
objects programmatically or after manipulating data in a `data.frame`:

```{r dataSetsFromDataFrame}
...
`dataSetsFromDataFrame()` can also be used to create `DataSet` objects from
a `data.frame` that was not produced by `dataSetToDataFrame()`, for example
when observed data is provided as a plain table. The required columns are
`name`, `xValues`, and `yValues`. All other standard columns are optional,
and any additional columns are interpreted as meta data.

```{r dataSetsFromDataFrameManual}
