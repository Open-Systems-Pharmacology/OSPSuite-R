...
To run a population simulation, the `Population` object created by the `createPopulation` method must be passed to the `runSimulations()` method:
...
simulationResults <- runSimulations(simulations = sim, population = myPopulation)
...
populationResults <- runSimulations(simulations = sim, population = myPopulation, simulationRunOptions = simRunOptions)
print(populationResults)
```

Simulated time-value pairs for a specific output from the `SimulationResults`-object returned by the `runSimulations` method can be accessed with the method `getOutputValues`. The user can provide either the path(s) of the output (which can be a molecule, a parameter, or an observer), 
