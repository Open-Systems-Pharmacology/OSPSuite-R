...
createBaseGrid <- function() {
  DataColumn$new(rSharp::clrNew("OSPSuite.Core.Domain.Data.BaseGrid", "baseGrid", getDimensionByName(ospDimensions$Time)))
}
createColumn <- function(baseGrid) {
  DataColumn$new(rSharp::clrNew("OSPSuite.Core.Domain.Data.DataColumn", "column", getDimensionByName(ospDimensions$`Concentration (mass)`), baseGrid$ref))
}
