test_that("macOS ARM64 SQLite fix converts VIEWs to TABLEs correctly", {
  skip_on_os(c("windows", "linux"))
  skip_if(Sys.info()[["machine"]] != "arm64", "Only runs on macOS ARM64")
...
test_that("macOS ARM64 SQLite fix produces consistent row counts", {
  skip_on_os(c("windows", "linux"))
  skip_if(Sys.info()[["machine"]] != "arm64", "Only runs on macOS ARM64")
...
test_that("macOS ARM64 SQLite fix marker is valid", {
  skip_on_os(c("windows", "linux"))
  skip_if(Sys.info()[["machine"]] != "arm64", "Only runs on macOS ARM64")
...
test_that("macOS ARM64 SQLite TABLE conversion produces same results as original VIEWs", {
  skip_on_os(c("windows", "linux"))
  skip_if(Sys.info()[["machine"]] != "arm64", "Only runs on macOS ARM64")
