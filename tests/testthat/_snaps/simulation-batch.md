# It can print simulation batch

    Code
      print(simBatch)
    Output
      <SimulationBatch>
      <ID>
      <ID>
      Variable parameters:
        * Organism|Volume
        * Organism|TableParameter
        * Organism|Q
        * Organism|RHSParameter
        * Organism|DistributedParameter
        * Organism|FormulaParameter
        * Organism|Liver|Volume
        * Organism|Liver|A|Concentration
        * Organism|Liver|B|Concentration
        * Organism|Liver|Dapagliflozin|Concentration
        * Organism|Liver|Metformin|Concentration
        * Organism|Lumen|Stomach|Volume
        * Organism|Lumen|Stomach|Dapagliflozin|Concentration
        * Organism|Lumen|Stomach|Metformin|Concentration
        * Organism|DistributedParameter|Percentile
        * Organism|DistributedParameter|Mean
        * Organism|DistributedParameter|Deviation
        * Organism|A|Concentration
        * Organism|B|Concentration
        * Organism|Dapagliflozin|Concentration
        * Organism|Metformin|Concentration
        * Dapagliflozin|Molecular weight
        * Metformin|Molecular weight
        * R1|k1
      Variable start values:
        * Organism|A
        * Organism|B
        * Organism|Dapagliflozin
        * Organism|Metformin
        * Organism|Liver|A
        * Organism|Liver|B
        * Organism|Liver|Dapagliflozin
        * Organism|Liver|Metformin
        * Organism|Lumen|Stomach|Dapagliflozin
        * Organism|Lumen|Stomach|Metformin
      Simulation:
        * Name: Simple
        * File path: ../data/simple.pkml
      <ID>

