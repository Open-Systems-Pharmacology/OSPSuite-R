...
volumeParameter <- getParameter(volumePath, simple)
distributedParameter <- getParameter("Organism|DistributedParameter", simple)
formulaParameter <- getParameter("Organism|FormulaParameter", simple)
constantParameter <- getParameter("Organism|Q", simple)
...
test_that("It can retrieve name of a parameter", {
  expect_equal(volumeParameter$name, "Volume")
})

test_that("It can display whether a parameter is a constant parameter", {
  expect_false(distributedParameter$isConstant)
...
test_that("It can display whether a parameter is a formula parameter", {
  expect_false(distributedParameter$isFormula)
...
test_that("It can retrieve the full path of a parameter", {
  expect_equal(
    volumeParameter$fullPath,
    paste(simple$name, volumePath, sep = "|")
  )
})
...
test_that("it can retrieve all units defined for a quantity", {
  expect_identical(volumeParameter$allUnits, c("l", "ml", .encodeUnit("µl")))
})

test_that("It returns NULL if the parameter has no RHS", {
  expect_null(volumeParameter$rhsFormula)
})

test_that("isStateVariable returns false if the parameter has no RHS Formula", {
  expect_false(volumeParameter$isStateVariable)
})
...
test_that("It throws an exception when setting the isStateVariable to true for a parameter that has no state variable", {
  expect_error(volumeParameter$isStateVariable <- TRUE)
})
...
test_that("It can set a value without the unit specified, thus using the default unit", {
  ...
  volumeParameter$setValue(10)
  expect_equal(volumeParameter$value, 10)
})

test_that("It can set the isStateVariable to false for a parameter without RHS", {
  volumeParameter$isStateVariable <- FALSE
  expect_false(volumeParameter$isStateVariable)
  expect_null(volumeParameter$rhsFormula)
})
