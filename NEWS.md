...
- Added `createSimulations()` to create several simulations from a named list of `SimulationConfiguration` objects in a single call, which builds them in parallel. A configuration that cannot be created is reported as a warning and its entry in the returned list is `NULL`; pass `stopIfFails = TRUE` to raise an error instead. `createSimulation()` is unchanged and now goes through the same batch API (#2024).
