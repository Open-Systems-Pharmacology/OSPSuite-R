...
- `projectToSnapshot()` and `snapshotToProject()` now convert MoBi projects (`.mbp3`) as well as PK-Sim projects (`.pksim5`). The application is detected from the file extension when converting a project and from the snapshot itself when converting a snapshot; the new `application` argument overrides the detection, and a single call may mix projects from both applications.
