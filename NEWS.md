...
- Added `loadSimulationsFromSnapshot()` to load simulations stored in a snapshot file as `Simulation` objects, optionally filtering by simulation name (#1929). Requesting a name that is not in the snapshot raises an error; pass `ignoreIfNotFound = TRUE` to return `NULL` for missing names instead. See `vignette("snapshots")` for an overview of the snapshot helpers. Note: snapshots containing PK-Sim modules cannot yet be converted in the headless R package (`PKSim.Starter.dll` is not shipped because it references UI assemblies); support is tracked upstream in PK-Sim and MoBi.
