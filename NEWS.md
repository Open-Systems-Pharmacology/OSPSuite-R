...
- `toDisplayUnit()` now accepts an optional `unit` parameter to specify the source unit of the values, consistent with `toUnit()` and `toBaseUnit()` functions. When not specified, values are assumed to be in base unit (maintaining backward compatibility).
