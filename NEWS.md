...
- `ospsuite` now installs and loads even when its native libraries or the .NET runtime cannot be initialised (for example on build machines that lack a compatible `libxml2` or the .NET runtime), instead of failing at load time. The reason is reported when the package is attached and raised with an actionable message on the first call into the .NET API, which allows the package to be built, checked, and resolved as a dependency in environments without a working runtime.
