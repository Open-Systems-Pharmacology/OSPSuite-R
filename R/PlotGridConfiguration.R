...
PlotGridConfiguration <- R6::R6Class(
  ...
  public = list(
    ...
    #' annotations, where plot refers to the grid of plots as a whole.
    #' @param tagLevels A character vector defining the enumeration format to
    #'   use at each level. Possible values are `'a'` for lowercase letters,
    #'   `'A'` for uppercase letters, `'1'` for numbers, `'i'` for lowercase
    #'   Roman numerals, and `'I'` for uppercase Roman numerals. It can also be
    #'   a list containing character vectors defining arbitrary tag sequences.
    #'   If any element in the list is a scalar and one of `'a'`, `'A'`, `'1'`,
    ...
    #' @param theme A ggplot theme specification to use for the plot. Only
    #'   elements related to the titles as well as plot margin and background
    #'   will be used.
    #' @param nColumns,nRows The dimensions of the grid to create - if both are
    #'   `NULL` it will use the same logic as
    #'   [facet_wrap()][ggplot2::facet_wrap] to set the dimensions.
    #' @param byRow Analogous to `byrow` in [matrix()][base::matrix]. If `FALSE`
    #'   the plots will be filled in in column-major order.
