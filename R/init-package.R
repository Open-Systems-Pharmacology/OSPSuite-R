...
if (.Platform$OS.type == "unix") {
  ...
  if (Sys.info()[['sysname']] == "Linux"){
    ...
    if (Sys.info()[['machine']] == "arm64") {
      for (arm64File in list.files(system.file("lib", package = ospsuiteEnv$packageName), pattern = "\\.dylib$", full.names = TRUE)) {
        file.copy(arm64File, gsub(".Arm64.dylib", ".dylib", arm64File))
      }
    }
    # if x64 architecture, rename *x64.dylib files to *.dylib
    else if (Sys.info()[['machine']] == "x86_64") {
      for (x64File in list.files(system.file("lib", package = ospsuiteEnv$packageName), pattern = "\\.dylib$", full.names = TRUE)) {
        file.copy(x64File, gsub(".x64.dylib", ".dylib",  x64File))
      }
    }
