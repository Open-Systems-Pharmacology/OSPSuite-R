...
``` r
expressionProfileBBToDataFrame(expressionProfilesBuildingBlock)
```
...
- expressionProfilesBuildingBlock:
  ...
  A single `BuildingBlock` object of type `Expression Profile`, or a
  list of such objects. When a list is provided, the resulting data
  frames are row-bound across all building blocks.
...
- `initialConditions`: A data frame with the following columns:
  ...
  - `Value`: Initial value of the molecule. For values that are defined
    by a formula, the return value can be `NaN`.
