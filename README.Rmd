...
parse_dep_field <- function(field) {
  if (is.null(field) || is.na(field)) return(character())
...
for (non_cran_dep in non_cran_deps) {
  pd <- utils::packageDescription(non_cran_dep)
  deps <- sort(unique(c(deps, parse_dep_field(pd$Depends), parse_dep_field(pd$Imports))))
}

deps <- deps[!deps %in% c("methods", "R", non_cran_deps)]
